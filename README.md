...
- `TODO_SECTIONS.md` - List of sections that need work (incomplete, orphaned, or missing files)
...
```bash
# Generate UNFINISHED_BRANCHES.md using the analysis script
uv run python scripts/generate_unfinished_branches.py sections UNFINISHED_BRANCHES.md
# or
python scripts/generate_unfinished_branches.py sections UNFINISHED_BRANCHES.md
```

The script analyzes all section files and identifies choices that point to non-existent target files.
...
The `TODO_SECTIONS.md` file identifies sections that need work to complete the story. It is generated from `DECISIONS.md` to help track what needs to be written or connected.

**What this provides:**
- **Incomplete Terminated Sections**: Sections marked as "Untitled" that are terminated (no outgoing links) - these need content written
- **Orphaned Sections**: Sections with no incoming links (unreachable through normal gameplay) - these need connections from other sections
- **Missing Section Files**: Sections referenced in `DECISIONS.md` but don't exist as files yet - these need to be created
- For each section, shows which sections reference it (for context when writing)
- Summary statistics of all sections needing work

**How to use it:**
1. **Check what needs work**: Open `TODO_SECTIONS.md` to see all sections that need attention
2. **Prioritize work**: Use the file to identify which sections to work on next
3. **Track progress**: As you complete sections, regenerate the file to see updated status
4. **Spawn agents**: Use the file to give agents clear tasks (write content, create files, add connections)

**How to generate it:**
```bash
# Generate both DECISIONS.md and TODO_SECTIONS.md together (recommended)
uv run python scripts/generate_all_docs.py
# or generate individually
uv run python scripts/generate_decisions.py
uv run python scripts/generate_todo_sections.py
# or
python scripts/generate_todo_sections.py
```

The script parses `DECISIONS.md` and cross-references with actual section files to identify:
- Sections that are incomplete (Untitled + terminated)
...
- **`DECISIONS.md`** - Comprehensive map of all section connections (see [Decisions Map](#decisions-map) above)
  ...
  uv run python scripts/generate_decisions.py
  ```
- **`TODO_SECTIONS.md`** - List of sections that need work (see [TODO Sections](#todo-sections) above)
  ```bash
  # Generate both DECISIONS.md and TODO_SECTIONS.md together
  uv run python scripts/generate_all_docs.py
  # or generate individually
  uv run python scripts/generate_decisions.py
  uv run python scripts/generate_todo_sections.py
...
**Note:** `TODO_SECTIONS.md` depends on `DECISIONS.md`, so always generate `DECISIONS.md` first before generating `TODO_SECTIONS.md`.
